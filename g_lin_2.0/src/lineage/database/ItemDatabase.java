...
import lineage.world.object.item.scroll.ScrollDominanceAmulet;
...
//			return RingOfTransform.clone(getPool(RingOfTransform.class)).clone(item);	
	...
	static private ItemInstance newDefaultItem(Item item) {
		...
		} else if (item.getType2().startsWith("지배 부적")) {
			// 이동 주문서
			return ScrollDominanceAmulet.clone(getPool(ScrollDominanceAmulet.class)).clone(item);
