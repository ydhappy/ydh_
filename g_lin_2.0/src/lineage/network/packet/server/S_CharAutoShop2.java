...
public class S_CharAutoShop2 extends ServerBasePacket {

	static synchronized public BasePacket clone(BasePacket bp, PcInstance pc, List<ItemInstance> list) {
		if (bp == null)
			bp = new S_CharAutoShop2(pc, list);
		else
			((S_CharAutoShop2) bp).toClone(pc, list);
		return bp;
	}

	public S_CharAutoShop2(PcInstance pc, List<ItemInstance> list) {
		toClone(pc, list);
	}
