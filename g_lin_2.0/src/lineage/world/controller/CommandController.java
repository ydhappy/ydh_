...
//						}
					...
					이펙트(o, st);
					return true;
				} else if (key.equalsIgnoreCase(Lineage.command + "자동파티사냥")) {
				    if (o instanceof PcInstance) {
				        PcInstance pc = (PcInstance) o;
				        String mode = st.hasMoreTokens() ? st.nextToken() : "";
				        
				        lineage.bean.lineage.Party p = PartyController.find(pc);
				        if (p == null || p.getMaster() == pc) {
				            ChattingController.toChatting(pc, "파티원 상태에서만 사용 가능한 명령어입니다.", Lineage.CHATTING_MODE_MESSAGE);
				            return true;
				        }

				        if (mode.equals("켜기")) {
				            pc.setPartyFollowMode(true);
				            pc.setNextPartyAiTime(0);
				            // 기존 isAutoHunt는 건드리지 않음 (중요)
				            ChattingController.toChatting(pc, "자동 파티 사냥 모드가 활성화되었습니다.", Lineage.CHATTING_MODE_MESSAGE);
				        } else if (mode.equals("끄기")) {
				            pc.setPartyFollowMode(false);
				            ChattingController.toChatting(pc, "자동 파티 사냥 모드가 해제되었습니다.", Lineage.CHATTING_MODE_MESSAGE);
				        }
				    }
				    return true;
				}		
