...
public class Tigus extends DwarfInstance {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp){
		if(pc.getLawful()<Lineage.NEUTRAL) {
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "tigusl"));
		}else{
		...
		synchronized (sync_dynamic) {
			...
			//	pc.toSender(S_Message.clone(BasePacketPooling.getPool(S_Message.class), 209)); // 창고 목록 
				...
				if (cnt == 0) {
				   ...
				   pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "tiguso")); 
				} else {
