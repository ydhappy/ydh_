...
public class Kamit extends QuestInstance {
    ...
    public void toTalk(PcInstance pc, ClientBasePacket cbp) {
        ...
        if (cadmus_q == null)
        	cadmus_q = QuestController.newQuest(pc, this, Lineage.QUEST_CADMUS);

        switch (cadmus_q.getQuestStep()) {
        case 1:
            pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Kamit1"));
            break;
        case 2:
            pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Kamit1b"));
           // cadmus_q.setQuestStep(3);
            break;
        default:	
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Kamit1"));
			break;
    ...
    private void giveItemToPC(PcInstance pc) {
        if (pc == null)
        	return;
        Item i = ItemDatabase.find("카드무스의 목걸이");
		if (i != null) {
			ItemInstance crown = ItemDatabase.newInstance(i);
			if (pc != null && Util.isDistance(this, pc, 4)) {
				// 인벤에 등록처리.
				pc.getInventory().append(crown, true);
				ChattingController.toChatting(pc, String.format("카미트가 당신에게 %s를 주었습니다.", i.getName()), Lineage.CHATTING_MODE_MESSAGE);
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Kamit2"));
			}
        }
    }
