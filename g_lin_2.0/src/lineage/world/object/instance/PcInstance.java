...
public class PcInstance extends Character {
	...
	// 방향별 좌표 변화량 (0:북, 1:북동, 2:동, 3:남동, 4:남, 5:남서, 6:서, 7:북서)
	...
	private long nextPartyAiTime = 0;
	...
	private boolean isPartyFollowMode = false; // 파티 추적 모드 전용 변수
	...
	public boolean isPartyFollowMode() { return isPartyFollowMode; }
	public void setPartyFollowMode(boolean b) { this.isPartyFollowMode = b; }
	public void setNextPartyAiTime(long time) {
	    this.nextPartyAiTime = time;
	}
	// 로직에서 사용할 Getter 메서드 (선택 사항이지만 권장)
	public long getNextPartyAiTime() {
	    return this.nextPartyAiTime;
	}
	...
	public void toTimer(long time) {
		...
		try {
	        if (this.isPartyFollowMode() && !this.isWorldDelete()) {
	            // 현재 서버 시간(time)이 다음 실행 시간보다 크거나 같을 때 실행
	            if (time >= nextPartyAiTime) {
	                // 다음 실행 시간은 현재 시간으로부터 0.4초 후로 설정
	                this.nextPartyAiTime = time + 200; 
	                this.executePartyFollowAi(time); 
	            }
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
...
*/
	...
	public void autoPotion() {
	    // 1. 공통 상태 체크 (죽음, 투명, 잠금 등)
	    if (getInventory() == null || isWorldDelete() || isInvis() || isTransparent() || isDead() || isLock() || isLockHigh() || isLockLow()) {
	        return;
	    }

	    // --- [HP 자동물약 로직] ---
	    int hpPercent = (int) (((double) getNowHp() / (double) getTotalHp()) * 100.0);
	    ItemInstance hpItem = null;

	    // 설정된 HP 퍼센트 이하일 때만 물약 찾기
	    if (autoPotionName != null && hpPercent <= autoPotionPercent) {
	        for (ItemInstance potion : getInventory().getList()) {
	            if (potion instanceof HealingPotion && potion.getItem().getName().equalsIgnoreCase(autoPotionName)) {
	                hpItem = potion;
	                break;
	            }
	        }
	    ...
	    if (hpItem != null) {
	        if (hpItem.isClick(this)) {
	            hpItem.toClick(this, null);
	            // HP가 위험한 상황이므로 마나 로직은 이번 턴에 실행하지 않고 종료
	            this.isMpRestoring = false; 
	            return;
	        }
	    } else if (autoPotionName != null && hpPercent <= autoPotionPercent) {
	        // 설정된 HP 물약이 바닥났을 때 알림
			...
			if (action.contains("autohunt-")) {
				...
				if (action.contains("skill-")) {
					...
					if (action.equalsIgnoreCase("on")) {
						...
						// 특정 아이템 코드를 가진 아이템이 착용 중인지 확인
					    boolean isSpecialItemEquipped = false;
					    for (ItemInstance item : this.getInventory().getList()) {
					        if (item.getItem().getName().equalsIgnoreCase("가더") && item.isEquipped()) { 
					            isSpecialItemEquipped = true;
					            break;
					        }
					    }
						...
						if (!isMap && !isSpecialItemEquipped) {
		...
		try {
			...
			if (getInventory() != null && autohunt_target != null && !isDead() && !isWorldDelete()) {
				...
				if (getNowHp() >= getMaxHp()) {
	                // 설정된 autoMPPercent2보다 마나가 낮으면 회복 모드 ON
	                if (getMpPercent() < autoMPPercent2) {
	                    this.isMpRestoring = true;
	                }

	                if (this.isMpRestoring) {
	                    long currentTime = System.currentTimeMillis();
	                    // 1초(1000ms) 딜레이 체크
	                    if (currentTime - getLastcooltime() >= 1000) {
	                        // 인벤토리에서 1374번 아이템 검색
	                        ItemInstance mpItem = null;
	                        for (ItemInstance item : getInventory().getList()) {
	                            if (item != null && item.getItem() != null && item.getItem().getItemCode() == 1374) {
	                                mpItem = item;
	                                break;
	                            }
	                        }

	                        if (mpItem != null && mpItem.isClick(this)) {
	                            mpItem.toClick(this, null);
	                            setLastcooltime(currentTime);
	                            
	                            // 아이템 사용 동작 시간 부여 (0.8초)
	                            ai_time = 800; 
	                            return true; // 아이템을 사용했으므로 이번 프레임은 여기서 종료
	                        }
	                    }

	                    // MP가 100%가 되면 회복 모드 종료
	                    if (getNowMp() >= getMaxMp()) {
	                        this.isMpRestoring = false;
	                    }
	                }
	            } else {
	                // 사냥 중 피가 1이라도 달면 즉시 중단 (안전 우선)
	                this.isMpRestoring = false;
	            }
	...
	public void executePartyFollowAi(long time) {
	    lineage.bean.lineage.Party p = lineage.world.controller.PartyController.find(this);
	    if (p == null) { this.setPartyFollowMode(false); return; }

	    PcInstance leader = p.getMaster();
	    if (leader == null || leader.getObjectId() == 0) return;

	    // 1. 텔레포트 (검증 완료)
	    double distToLeader = lineage.util.Util.getDistance(getX(), getY(), leader.getX(), leader.getY());
	    if (leader.getMap() != getMap() || distToLeader > 20) {
	        this.toTeleport(leader.getX(), leader.getY(), leader.getMap(), false);
	        return;
	    }

	    // 2. 공격 대상 인식 강화 (리더의 타겟을 실시간으로 가져옴)
	    lineage.world.object.Character target = null;
	    if (leader.autoAttackTarget != null) {
	        target = (lineage.world.object.Character) leader.autoAttackTarget;
	    }

	    // 공격 사거리 및 무기 체크
	    double attackRange = getAttackRange();

	    if (target != null && !target.isDead() && target.getMap() == getMap()) {
	        this.autoAttackTarget = target;
	        double distToTarget = lineage.util.Util.getDistance(getX(), getY(), target.getX(), target.getY());
	        
	        if (distToTarget <= attackRange) {
	            handleAutoAttack(target);
	        } else {
	            // 공격을 위한 실시간 추격
	            synchronizedMove(target.getX(), target.getY(), "공격추격");
	        }
	    } else {
	        // 3. 리더 실시간 추격
	        this.autoAttackTarget = null;
	        if (distToLeader >= 2.0) { // 리더와 거리가 2칸 이상이면 즉시 추격
	            synchronizedMove(leader.getX(), leader.getY(), "리더추격");
	        }
	    }
	}
	 ...
	 * [해결] 실시간 이동 및 화면 동기화
	 */
	private void synchronizedMove(int tx, int ty, String mode) {
	    // 딜레이를 150~180ms로 조정 (가속 상태 고려, 실시간 이동처럼 보이게 함)
	    if (System.currentTimeMillis() - this.getMovingTime() < 160) return;

	    int dir = lineage.util.Util.calcheading(getX(), getY(), tx, ty);
	    if (dir >= 0 && dir < 8) {
	        int nextX = getX() + DELTA_X[dir];
	        int nextY = getY() + DELTA_Y[dir];

	        // 서버 좌표 갱신
	        lineage.world.World.remove(this); 
	        this.setX(nextX);
	        this.setY(nextY);
	        this.setHeading(dir);
	        lineage.world.World.append(this);

	        // [핵심] 화면 이동 동기화 패킷
	        // 1. 본인 화면을 한 칸 이동시킴
	        lineage.network.packet.BasePacket movePtr = lineage.network.packet.server.S_ObjectMoving.clone(
	            lineage.network.packet.BasePacketPooling.getPool(lineage.network.packet.server.S_ObjectMoving.class), this);
	        this.toSender(movePtr);           
	        
	        // 2. 주변에 나를 보여줌
	        lineage.world.World.toSender(movePtr); 

	        this.setMovingTime(System.currentTimeMillis());
	    }
	}

	/**
	 * [해결] 공격 미동작 해결 (타겟 정보 및 패킷 강제 송신)
	 */
	private void handleAutoAttack(lineage.world.object.Character target) {
	    if (target == null || target.isDead()) return;

	    long currentTime = System.currentTimeMillis();
	    // 공격 속도 보정 (리더와 비슷하게 칼질하도록 600~800ms 사이 조정)
	    if (currentTime - lastAttackTime < 600) return; 

	    // 방향 전환 패킷 전송
	    int dir = lineage.util.Util.calcheading(getX(), getY(), target.getX(), target.getY());
	    this.setHeading(dir);

	    boolean isBow = getAttackRange() > 5.0;
	    int effect = isBow ? 66 : 0;

	    // S_ObjectAttack 패킷 (사용자님의 소스 규격에 맞춤)
	    lineage.network.packet.BasePacket attackPtr = lineage.network.packet.server.S_ObjectAttack.clone(
	        lineage.network.packet.BasePacketPooling.getPool(lineage.network.packet.server.S_ObjectAttack.class), 
	        this, target, lineage.share.Lineage.GFX_MODE_ATTACK, 5, effect, isBow, true, target.getX(), target.getY());
	    
	    // 본인과 월드에 동시 전송 (그래야 칼질 모션이 보임)
	    this.toSender(attackPtr);
	    lineage.world.World.toSender(attackPtr);

	    // 실제 서버 데이터상 대미지 적용
	    target.toDamage(this, 10, 0, (Object[])null); 
	...
	private double getAttackRange() {
	    for (lineage.world.object.instance.ItemInstance item : this.getInventory().getList()) {
	        if (item != null && item.isEquipped() && item.getItem() != null) {
	            if ("20".equals(item.getItem().getType1()) || "bow".equalsIgnoreCase(item.getItem().getType2())) {
	                return 13.0;
	            }
	        }
	    }
