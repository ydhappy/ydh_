(() => {
  ...
  function badgeText() {
    ...
    if (current.hasToken) return 'AUTH TOKEN ON';
  ...
  function render(error = '') {
    ...
    panel.innerHTML = `
      <div class="server-auth-head">
        <div>
          ...
          <h3>서버 계정 인증</h3>
          <p>토큰 로그인 후 서버 저장, custom map, 계정/캐릭터 API 요청에 자동 인증 헤더를 붙입니다.</p>
        </div>
    ...
    panel.querySelector('#serverAuthLogout')?.addEventListener('click', () => {
      auth.logout();
      render();
    });
  }
