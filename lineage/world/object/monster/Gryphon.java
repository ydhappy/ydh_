...
public class Gryphon extends MonsterInstance {
	...
	public void toAi(long time) {
		...
		if (getGfxMode() == getClassGfxMode()) {
			if (getNowHp() <= (getTotalHp() * 0.4) && Util.random(0, 99) <= 15)
				toStay(true);
		}
	...
	public void toDamage(Character cha, int dmg, int type, Object...opt) {
		...
		if (SpriteFrameDatabase.findGfxMode(getGfx(), getGfxMode() + 44)) {
			return;
		}
		toStay(false);
	}
	...
	public void toAiAttack(long time) {
		...
		if (isItemInside()) {
			if (SpriteFrameDatabase.findGfxMode(getGfx(), getGfxMode() + 44)) {
			//ai_time = SpriteFrameDatabase.find(gfx, gfxMode + 44);
			toStay(false);
			}
	    }
	}
	...
	private boolean isItemInside() {
		for (object o : getInsideList(true)) {
			if (isPickupItem(o) && Util.isDistance(this, o, 8))
				return item_list.contains(((ItemInstance) o).getItem().getNameIdNumber());
		}
		return false;
	}
