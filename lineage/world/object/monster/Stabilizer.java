...
public class Stabilizer extends MonsterInstance {
	...
	public Stabilizer() {
		quest_item = ItemDatabase.find("스타이저의 증표");
	}
		...
		if (quest_item != null && getAttackListSize() == 1) {
			...
			if (o instanceof PcInstance && !(o instanceof RobotInstance)) {
				...
				if (q != null && q.getQuestStep() == 3) {
					ItemInstance Secret_Book = pc.getInventory().find(ItemDatabase.find("스타이저의 증표"));
