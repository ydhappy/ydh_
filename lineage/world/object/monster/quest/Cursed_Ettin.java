...
public class Cursed_Ettin extends MonsterInstance {
	...
	protected void toAiWalk(long time){
	    ...
	    if(toSearchHuman()) {
	        ChattingController.toChatting(this, "자극 하지 마라! 더 이상 강요 하지 마라!", Lineage.CHATTING_MODE_SHOUT);
	        new Thread(() -> {
	            try {
	                Thread.sleep(10000); // 10초 대기
	                ChattingController.toChatting(this, "미친 군주가 보낸 파수군이냐? 죽어버려!!", Lineage.CHATTING_MODE_SHOUT);
	    ...
	    if (quest_item != null && getAttackListSize() == 1) {
	        ...
	        if (o instanceof PcInstance && !(o instanceof RobotInstance)) {
	            ...
	            if (q != null && q.getQuestStep() == 0) {
	                ItemInstance Secret_Book = pc.getInventory().find(ItemDatabase.find("붉은 열쇠"));
