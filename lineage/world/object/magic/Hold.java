...
public class Hold extends Magic {
	...
	static private void toBuff(Character cha, Skill skill, int action, int area, int effect, double alpha_dmg) {
	    ...
	    // 5초 딜레이 후 실행될 작업을 타이머로 설정
	    Timer timer = new Timer();
	    TimerTask delayedTask = new TimerTask() {
	        @Override
	        public void run() {
	            cha.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), cha, 4848), true);
	            cha.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), cha, 4849), true);
	        }
	    };
	    // 5초(5000밀리초) 딜레이 설정
