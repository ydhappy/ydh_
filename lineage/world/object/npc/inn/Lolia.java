...
public class Lolia extends InnInstance {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp){
		pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia"));
	}
		...
		if(action.equalsIgnoreCase("room")){
			...
			if(pc.getLawful()>=Lineage.NEUTRAL){
				if(list.size()<Lineage.inn_max){
					if(find(pc) != null){
						// 이미 빌렷다는 창 띄우기
						pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia5"));
					}else{
						if(Lineage.server_version <= 144){
							...
							// 여관키 구입창 띄우기
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "Lolia2", action, Lineage.inn_price, 1, 1, Lineage.inn_in_max, list_hypertext));
						}
					}
				}else{
					// 이미 방이 꽉 찼다는거 띄우기.
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia6"));
				}
			}else{
				// 카오에게 보낼 메세지
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia1"));
			}
			...
			if(pc.getLawful()>=Lineage.NEUTRAL){
				if(pc.getClassType() == Lineage.LINEAGE_CLASS_ROYAL){
					if(list_hall.size()<Lineage.inn_hall_max){
						if(findHall(pc) != null){
							// 이미 빌렷다는 창 띄우기
							pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia15"));
						}else{
							if(Lineage.server_version <= 144){
								...
								// 여관키 구입창 띄우기
								pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "Lolia12", action, Lineage.inn_hall_price, 1, 1, Lineage.inn_hall_in_max, list_hypertext));
							}
						}
					}else{
						// 이미 방이 꽉 찼다는거 띄우기.
						pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia16"));
					}
				}else{
					// 홀은 왕자나 공주님만이 대여할 수 있습니다.
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia10"));
				}
			}else{
				// 카오에게 보낼 메세지
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia11"));
			}
			...
			if(find(pc)!=null || findHall(pc)!=null){
				...
				if(return_total_aden > 0){
					...
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia20", null, list_hypertext));
				}
			}
			...
			if (ik != null) {
			    ...
			    pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia21"));
			}
	...
	private void toInn1(PcInstance pc, List<InnKey> list, String action, int count, int inn_in_max, int inn_max, int inn_price){
		...
		if(pc.getInventory().isAden(count*inn_price, true)){
			...
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia4"));
		}else{
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "Lolia3"));
		}
	}
