...
//						}
	...
	/**
	 * ".서먼 다미 안타라스 10" 등 명령어 파싱 및 실제 소환 처리
	 */
	private static void toSummon(Object o, StringTokenizer st) {
	    ...
	    try {
	        ...
	        ChattingController.toChatting(issuer, "[시스템] 명령어가 올바르지 않습니다. 예) .서먼 다미 안타라스 10", Lineage.CHATTING_MODE_MESSAGE);
	    } catch (Exception e) {
	        e.printStackTrace();
	...
	private static void toDismissSummon(object o, StringTokenizer st) {
	    ...
	    if (!st.hasMoreTokens()) {
	        ChattingController.toChatting(issuer, "[시스템] 명령어가 올바르지 않습니다. 예) .서먼해산 다미", Lineage.CHATTING_MODE_MESSAGE);
	        return;
	    }
