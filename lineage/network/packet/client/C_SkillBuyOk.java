...
public class C_SkillBuyOk extends ClientBasePacket {
	...
	private void processSkillsInMap16(PcInstance pc, List<Skill> skillList) {
		...
		for (Skill skill : skillList) {
			...
			if (processSkill(pc, pcSkills, skill, firstTierSkills, "미스릴", 50, "판의 갈기털", 10, "페어리 더스트", 100, "아라크네의 거미줄", 10))
				continue;
			if (processSkill(pc, pcSkills, skill, secondTierSkills, "미스릴 실", 10, "버섯포자의 즙", 8, "판의 뿔", 1, "엔트의 껍질", 3))
				continue;
			processSkill(pc, pcSkills, skill, thirdTierSkills, "오리하루콘", 45, "아라크네의 허물", 3, "판의 뿔", 3, "페어리의 날개", 3);
		}

		SkillController.sendList(pc);
	}

	private boolean processSkill(PcInstance pc, List<Skill> pcSkills, Skill skill, String[] skillNames, String itemName1, int quantity1, String itemName2, int quantity2, String itemName3, int quantity3, String itemName4,
			int quantity4) {

		for (String skillName : skillNames) {
			if (skill.getName().startsWith(skillName)) {
				if (removeItemsAndAddSkill(pc, pcSkills, skill, itemName1, quantity1) || removeItemsAndAddSkill(pc, pcSkills, skill, itemName2, quantity2)
						|| removeItemsAndAddSkill(pc, pcSkills, skill, itemName3, quantity3) || removeItemsAndAddSkill(pc, pcSkills, skill, itemName4, quantity4)) {
					return true;
				} else {
					ChattingController.toChatting(pc, String.format("마법 %s 배우기 위해 지불할 재료가 부족합니다.", Util.getStringWord(skill.getName(), "을", "를")), Lineage.CHATTING_MODE_MESSAGE);
					return false;
				}
			}
		}
