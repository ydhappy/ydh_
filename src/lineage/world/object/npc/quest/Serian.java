...
public class Serian extends FirstQuest {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		case 0:
			...
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb2"));
		...
		case 1:
			if (pc.getLevel() >= 5) {
				...
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb14"));
			}
			break;
		case 2:
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb6"));
			break;
		case 3:
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb8"));
			break;
		case 4:
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb10"));
			break;
		case Lineage.QUEST_END:
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb11"));
			break;
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
		...
		ItemInstance Cuckoos = pc.getInventory().find(ItemDatabase.find("스타이저의 증표"));
		ItemInstance Certificate = pc.getInventory().find(ItemDatabase.find("섬의 증표"));
		...
		if (action.equalsIgnoreCase("0")) {
			...
			if (q != null && q.getQuestStep() == 1) {
				...
				if (CraftController.isCraft(pc, l, true)) {
					...
					if (pc.getInventory().find("상아탑의 가죽 투구") != null || pc.getInventory().find("상아탑의 가죽 장갑") != null) {
						q.setQuestStep(2);
						pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb9-2"));
					} else {
						// 지도가 없을 경우 아이템 지급 로직
						toStep1(pc);
						q.setQuestStep(2);
						// 창 띄우기.
						pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb5"));
					}
				}
			}
			...
			if (q != null && q.getQuestStep() == 2) {
				if (Secret_Book != null) {
					if (pc.getClassType() == Lineage.LINEAGE_CLASS_ROYAL) {
						...
						CraftController.toCraft(this, pc, ItemDatabase.find("상아탑의 악마의 피"), 10, true);
					} else if (pc.getClassType() == Lineage.LINEAGE_CLASS_WIZARD) {
						CraftController.toCraft(this, pc, ItemDatabase.find("상아탑의 가죽 갑옷"), 1, true);
						CraftController.toCraft(this, pc, ItemDatabase.find("상아탑의 체력 회복제"), 100, true);
						CraftController.toCraft(this, pc, ItemDatabase.find("상아탑의 속도향상 물약"), 10, true);
						CraftController.toCraft(this, pc, ItemDatabase.find("상아탑의 지혜의 물약"), 10, true);
					}
					pc.getInventory().remove(Secret_Book, true);
					q.setQuestStep(3);
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb7"));
				} else {
			...
			if (q != null && q.getQuestStep() == 3) {
				if (Cuckoos != null) {
					...
					CraftController.toCraft(this, pc, ItemDatabase.find("지도 말섬"), 1, true);
					pc.getInventory().remove(Talking_scroll, true);
					pc.getInventory().remove(Cuckoos, true);
					q.setQuestStep(4);
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "orenb9"));
				} else {
					// 퀘스트 아이템이 없을 때 메시지 전송
					ChattingController.toChatting(pc, "스타이저의 증표 아이템이 부족합니다.", 20);
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, ""));
				}
			}
			...
			if (q != null && q.getQuestStep() == 4) {
				if (Certificate != null) {
					...
					// 퀘스트 아이템이 없을 때 메시지 전송
					ChattingController.toChatting(pc, "섬의 증표 아이템이 부족합니다.", 20);
					pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, ""));
				}
			}
		}
