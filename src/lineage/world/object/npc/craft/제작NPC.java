...
public class 제작NPC extends CraftInstance {

	public 제작NPC(Npc npc) {
		super(npc);
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		if (Lineage.is_user_store) {
			if (pc.getLawful() < Lineage.NEUTRAL)
				pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "cnpc2"));
		} else {
			pc.toSender(S_Html.clone(BasePacketPooling.getPool(S_Html.class), this, "cnpc1"));
		}
	}
				...
				if (action.equalsIgnoreCase("request hard leather")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA1", action, 0, 1, 1, max, temp_request_list));
				...
				if (action.equalsIgnoreCase("request leather cap")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA2", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather sandal")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA3", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather vest")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA4", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather shield")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA5", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather boots")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA6", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather helmet")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA7", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request hard leather vest")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA8", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request leather vest with belt")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA9", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
			if (craft != null) {
				...
				if (action.equalsIgnoreCase("request belt")) {
					...
					if (CraftController.isCraft(pc, list.get(craft), true)) {
						...
						if (Lineage.server_version <= 144)
							...
							// 패킷 처리.
							pc.toSender(S_HyperText.clone(BasePacketPooling.getPool(S_HyperText.class), this, "cnpcA10", action, 0, 1, 1, max, temp_request_list));
					}
				}
			}
		}
