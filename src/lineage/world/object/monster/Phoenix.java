...
public class Phoenix extends MonsterInstance {
	...
	public void toAiAttack(long time) {
	    if (!HP90 && getNowHp() <= getTotalHp() * 0.9) {
	        ChattingController.toChatting(this, "강력한 나의 화염이여! 모두 태워라!", Lineage.CHATTING_MODE_SHOUT);
	        HP90 = true;
	        return;
	    }

	    if (!HP40 && getNowHp() <= getTotalHp() * 0.8) {
	        ChattingController.toChatting(this, "불타 소멸해라.. 모두 타버려라!! 끼아아!", Lineage.CHATTING_MODE_SHOUT);
	        HP40 = true;
	        return;
	    }

	    if (!HP10 && getNowHp() <= getTotalHp() * 0.1) {
	        ChattingController.toChatting(this, "끼야아!! 끼아!", Lineage.CHATTING_MODE_SHOUT);
	        HP10 = true;
	        return;
	    }
