...
public class Perez extends MonsterInstance {
	...
	public void toAiAttack(long time) {
	    if (!HP90 && getNowHp() <= getTotalHp() * 0.9) {
	        ChattingController.toChatting(this, "이 힘에 맞설 수 있겠느냐.", Lineage.CHATTING_MODE_SHOUT);
	        HP90 = true;
	        return;
	    }

	    if (!HP40 && getNowHp() <= getTotalHp() * 0.8) {
	        ChattingController.toChatting(this, "최후를 맞이 하거라!", Lineage.CHATTING_MODE_SHOUT);
	        HP40 = true;
	        return;
	    }

	    if (!HP10 && getNowHp() <= getTotalHp() * 0.1) {
	        ChattingController.toChatting(this, "어리석구나 이 저주는 영원할 것이다!", Lineage.CHATTING_MODE_SHOUT);
	        HP10 = true;
	        return;
	    }
	...
	protected void toAiWalk(long time){
		super.toAiWalk(time);
		if(toSearchHuman())
			ChattingController.toChatting(this, "어둠이 모든 것을 삼킬 것이다.", Lineage.CHATTING_MODE_SHOUT);
		return;
	}
