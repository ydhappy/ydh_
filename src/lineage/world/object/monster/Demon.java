...
public class Demon extends MonsterInstance {
	...
	public void toAiAttack(long time) {
	    if (!HP90 && getNowHp() <= getTotalHp() * 0.9) {
	        ChattingController.toChatting(this, "나의 뜨거운 숨결을 버틸 수 있겠느냐!", Lineage.CHATTING_MODE_SHOUT);
	        HP90 = true;
	        return;
	    }

	    if (!HP40 && getNowHp() <= getTotalHp() * 0.8) {
	        ChattingController.toChatting(this, "너희들이 영웅인 줄 아느냐!", Lineage.CHATTING_MODE_SHOUT);
	        HP40 = true;
	        return;
	    }

	    if (!HP10 && getNowHp() <= getTotalHp() * 0.1) {
	        ChattingController.toChatting(this, "난 결코 쓰러지지 않는다!", Lineage.CHATTING_MODE_SHOUT);
	        HP10 = true;
	        return;
	    }
	...
	protected void toAiWalk(long time){
		super.toAiWalk(time);
		if(toSearchHuman())
			ChattingController.toChatting(this, "하찮은 인간들이여, 모두 죽여주마!", Lineage.CHATTING_MODE_SHOUT);
		return;
	}
