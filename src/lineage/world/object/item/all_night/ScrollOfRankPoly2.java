...
public class ScrollOfRankPoly2 extends ItemInstance {
	
	static synchronized public ItemInstance clone(ItemInstance item){
		if(item == null)
			item = new ScrollOfRankPoly2();
		return item;
	}
		...
		if (!cha.isDead() && !cha.isWorldDelete() && cha.getInventory() != null && cha instanceof PcInstance) {
			...
			if (!World.isTeamBattleMap(cha) && !cha.isFishing()) {
				...
				if (p != null) {
					...
					cha.toSender(S_ObjectEffect.clone(BasePacketPooling.getPool(S_ObjectEffect.class), cha, Lineage.poly_effect), true);
