...
`tiled-map-bootstrap.js`는 아래 기능을 제공합니다.
...
1. TILED MAP MANAGER의 textarea에 Tiled JSON 원문 붙여넣기
2. `붙여넣기 JSON 등록` 클릭
3. 검증 성공 시 맵 카드가 추가됨
4. `이 맵으로 이동` 클릭
...
- 붙여넣기 custom map은 localStorage에만 저장되며 서버 DB에는 아직 저장하지 않습니다.

## 22-5 다음 작업

다음 단계에서 진행합니다.

1. GM 콘솔에 source/sourceUrl 표시
2. 검증 실패 맵 상세 보기
3. Tiled object layer 기반 NPC/몬스터/포탈 배치
